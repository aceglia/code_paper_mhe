...
trials = ["abd_result", "flex_result", "cycl_result",
          "abd_cocon_result", "flex_cocon_result", "cycl_cocon_result"]
        ...
        for v, variable in enumerate(rmse_variables):
            ...
            if variable == "q":
                ...
                for i in range(markers_ref.shape[2]):
                    markers_tmp[:, :, i] = np.array(
                        [mark.to_array() for mark in model.markers(q[:, i])]).T
...
print('\ begin{tabular}[c]{cc|cc|cc|cc|cc|cc|cc}\n'
