...
plt.gcf().subplots_adjust(left=0.06, right=0.99, wspace=0.05, hspace=0.25, bottom=0.08, top=0.95)
    ...
    for i in range(models[0].nbMuscles()):
        ...
        if i not in not_interest_muscle:
            ...
            if count in [9, 10, 11]:
                ...
                # print x label using vector
                # plt.xticks(result_mat[s]["X_est"][6, :] * 57.3, fontsize=fontsize)
                # plt.xticks(result_mat[s]["f_est"][i, :], result_mat[s]["f_est"][i, :])
                #
                fig.set_xlim(0, tf)
            else:
                fig.set_xticklabels([])
            ...
            # plt.title(models[0].muscleNames()[i].to_string(), fontsize=fontsize)
            count += 1
